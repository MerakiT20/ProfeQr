...
(function(){
  ...
  function buildPayload(date){
    ...
    const c=cfg();
    return {
      school_id:c.schoolId||'school-unconfigured',
      report_date:date,
      group_name:c.groupName||db?.group?.name||db?.group?.groupName||'Grupo',
      teacher_id:c.teacherId||'teacher-unconfigured',
  ...
  function queue(payload){const out=readJson(OUTBOX_KEY,[]);out.push({id:Date.now()+'-'+Math.random().toString(36).slice(2),payload});writeJson(OUTBOX_KEY,out);}

  async function send(payload){
    const c=cfg();
    if(!c.supabaseUrl||!c.anonKey||!c.schoolId) throw new Error('SYNC_NOT_CONFIGURED');
    const url=c.supabaseUrl.replace(/\/$/,'')+'/rest/v1/attendance_reports?on_conflict=school_id,report_date,group_name';
    const res=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json','apikey':c.anonKey,'Authorization':'Bearer '+c.anonKey,'Prefer':'resolution=merge-duplicates,return=minimal'},body:JSON.stringify(payload)});
  ...
  async function submit(date){
    ...
    setReportMeta(date,{status:'sending',at:new Date().toISOString()});
    try{
      await send(payload);
      setReportMeta(date,{status:'sent',at:new Date().toISOString()});
      if(typeof toast==='function')toast('Reporte enviado a Dirección ✓');
    }catch(e){
      queue(payload);
      setReportMeta(date,{status:'queued',at:new Date().toISOString()});
      if(typeof toast==='function')toast(e.message==='SYNC_NOT_CONFIGURED'?'Reporte guardado; falta configurar enlace con Dirección':'Sin conexión: reporte guardado para sincronizar');
    }
    if(typeof renderCurrentScreen==='function')renderCurrentScreen();
  }

  function statusHtml(date){
    const m=reportMeta()[date];
    if(!m)return '<span class="badge bad">Pendiente de reportar</span>';
  ...
  function reportCard(date){
    ...
    return `<div class="card no-print" id="att-direction-card">
      ...
      <div style="display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap">
        <div>${statusHtml(date)}<div class="small" style="margin-top:6px">${rows.length} presentes · ${absent} ausentes · ${students.length} total</div></div>
        <button class="btn primary" id="att-submit-director" ${isExpired()?'disabled':''}>Enviar reporte</button>
      </div>
      ...
      <div class="small" style="margin-top:8px">Pase rápido registra a todos; después toca únicamente a quienes faltaron.</div>
    </div>`;
  }

  function configure(){
    const c=cfg();
    const supabaseUrl=prompt('URL de Supabase',c.supabaseUrl||''); if(supabaseUrl===null)return;
    ...
    writeJson(CFG_KEY,{supabaseUrl:supabaseUrl.trim(),anonKey:anonKey.trim(),schoolId:schoolId.trim(),teacherName:teacherName.trim(),groupName:groupName.trim(),teacherId:teacherId.trim()});
    toast('Enlace guardado en este dispositivo');flush();
  }

  if(typeof renderAttendance==='function'){
    const originalRender=renderAttendance;
    renderAttendance=function(){return originalRender()+reportCard(attendanceDate);};
  }
  if(typeof bindAttendance==='function'){
    const originalBind=bindAttendance;
    bindAttendance=function(){
      originalBind();
