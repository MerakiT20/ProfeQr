...
function normalizeGuardCommission(g={}){
  ...
  x.title = String(x.title || (x.kind === 'Guardia' ? 'Guardia escolar' : x.kind === 'Honores' ? 'Honores a la bandera' : 'Comisión escolar'));
  x.guardType = String(x.guardType || '');
  x.zone = String(x.zone || '');
  x.commissionType = String(x.commissionType || '');
  ...
  x.groupName = String(x.groupName || '');
  x.date = normalizeIsoDate(x.date,today());
  x.startTime = String(x.startTime || '');
  x.endTime = String(x.endTime || '');
  ...
  x.expectedProduct = String(x.expectedProduct || '');
  x.observations = String(x.observations || '');
...
function ensureGuardCommissions(){ db.group.guardCommissions = Array.isArray(db.group.guardCommissions) ? db.group.guardCommissions.filter(isRecordObject).map(normalizeGuardCommission) : []; ensureAgenda(); }
